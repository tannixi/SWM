...
from neural_methods.model.VSWM import VSWM
from neural_methods.trainer.BaseTrainer import BaseTrainer
from tqdm import tqdm


class VSWMTrainer(BaseTrainer):
    ...
    def __init__(self, config, data_loader):
        ...
        if config.TOOLBOX_MODE == "train_and_test":
            os.environ['CUDA_VISIBLE_DEVICES'] = '1,2,3'  # 代表使用的第一块，第三块，第四块和第五块卡
            device_ids = [0,1,2]
            torch.cuda.get_device_name()
            self.model = VSWM(frame_depth=self.frame_depth, img_size=224)
            ...
            self.model = VSWM(frame_depth=self.frame_depth, img_size=224).to(
                self.device)
    ...
    def train(self, data_loader):
        ...
        for epoch in range(self.max_epoch_num):
            ...
            for idx, batch in enumerate(tbar):
                ...
                # optimizer.step()
                # 根据梯度更新网络参数
                # print('PPG,shape', pred_ppg.shape)
                loss = self.criterion(pred_ppg, labels)
                loss.backward()
                self.optimizer.step()
                self.scheduler.step()
                running_loss += loss.item()
                if idx % 107 == 106:  # print every 100 mini-batches
                    print(f'[{epoch}, {idx + 1:5d}] loss: {running_loss / 107:.5f}')
                    running_loss = 0.0
