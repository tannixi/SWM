...
from neural_methods.model.SM import SM
from neural_methods.trainer.BaseTrainer import BaseTrainer
from tqdm import tqdm


class SMTrainer(BaseTrainer):
    ...
    def __init__(self, config, data_loader):
        ...
        if config.TOOLBOX_MODE == "train_and_test":
            os.environ['CUDA_VISIBLE_DEVICES'] = '0,1,2'  # 代表使用的第一块，第三块，第四块和第五块卡
            device_ids = [0,1,2]

            self.model = SM(frame_depth=self.frame_depth, img_size=128)
            ...
            self.model = SM(frame_depth=self.frame_depth, img_size=128).to(
                self.device)
    ...
    def test(self, data_loader):
        ...
        if self.config.TOOLBOX_MODE == "only_test":
            ...
            for a in range(0,200):
                ...
                i=path_raw.split('/')[9].split('.')[0].split('Epoch')[1]
                b=int(i)+a
