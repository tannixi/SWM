...
from neural_methods.model.SWM import SWM
from neural_methods.trainer.BaseTrainer import BaseTrainer
from tqdm import tqdm

from PIL import Image
class SWMTrainer(BaseTrainer):
    ...
    def __init__(self, config, data_loader):
        ...
        if config.TOOLBOX_MODE == "train_and_test":
            os.environ['CUDA_VISIBLE_DEVICES'] = '0,1,2,3'  # 代表使用的第一块，第三块，第四块和第五块卡
            device_ids = [0,1,2,3]

            self.model = SWM(frame_depth=self.frame_depth, img_size=128)
            ...
            os.environ['CUDA_VISIBLE_DEVICES'] = '0'  # 代表使用的第一块，第三块，第四块和第五块卡
            device_ids = [0]
            self.model = SWM(frame_depth=self.frame_depth, img_size=128).to(
                self.device)
            self.model = torch.nn.DataParallel(self.model, device_ids=device_ids).cuda()
        else:
    ...
    def train(self, data_loader):
        ...
        for epoch in range(self.max_epoch_num):
            ...
            for idx, batch in enumerate(tbar):
                ...
                # save_name = '/data/tannixi_datas/feature_map/' + 'raw3' + '.png'
                ...
                # 为EfficientPhys再添加一个帧，因为它为输入执行torch.diff
                ...
                if idx % 83 == 82:  # print every 100 mini-batches
                    print(f'[{epoch}, {idx + 1:5d}] loss: {running_loss / 83:.5f}')
                    running_loss = 0.0
    ...
    def test(self, data_loader):
        ...
        if self.config.TOOLBOX_MODE == "only_test":
            ...
            for a in range(0,200):
                ...
                path_raw=self.config.INFERENCE.MODEL_PATH
                i=path_raw.split('/')[7].split('.')[0].split('Epoch')[1]
                b=int(i)+a
                path=path_raw.replace(str(i),str(b))
                a=a+1
