...
def calculate_metric_per_video(predictions, labels, fs=30, diff_flag=True, use_bandpass=True, hr_method='FFT'):
    ...
    if use_bandpass:
        ...
        # a=predictions.get('subject24')
        # pr=a.get(0).cpu()
        # la=labels.get('subject24').get(0).cpu()
        fig, ax = plt.subplots()
        x = np.linspace(0, 180, 180)
        y2 = predictions[360:540]
        y1=labels[360:540]
        ax.plot(x, y1,y2, label='linear')
        plt.xlabel('time')
        plt.ylabel('rPPG')
        plt.legend(["labels-rPPG","predictions-rPPG"], loc='upper right')
        plt.savefig('/home/tannixi/1.png')# 作y1 = x 图，并标记此线名为linear
        plt.show()
        # print(predictions.get('subject24').get('0'))
